...
let ticketPriceTotal = runners.reduce(function(allDonations, donationTotal) {
    return allDonations + donationTotal.donation;
}, 0
);
...
// Problem 1: Change company names to all capitals
let companyCaps = runners.map(function(uppercase) {
    console.log(uppercase.company_name.toUpperCase());
});

// Problem 2: List of all runners who work for Gigashots
let gigashotsRunners = runners.filter(function(runner) {
    return runner.company_name === "Gigashots";
});
   console.log(gigashotsRunners);

// Problem 3: Convert runners last name and email and donation amount into a new array
let nameEmail = []
runners.forEach(function(runner) {
    return nameEmail.push(`Name: ${runner.last_name} | Email: ${runner.email} | Donation Amount: $${runner.donation} `)
});
